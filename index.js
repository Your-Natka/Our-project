import{n as e}from"./assets/rolldown-runtime-BDqkOrsv.js";import"./assets/styles-D6AMAj2K.js";import{t}from"./assets/vendor-BQcEvRoX.js";var n=e(t(),1),r=`https://your-energy.b.goit.study/api`;async function i(e){let t=await fetch(`${r}${e}`);if(!t.ok)throw Error(`HTTP error! Status: ${t.status}`);return t.json()}function a(e,t=1,n=12){return i(`/filters?filter=${e}&page=${t}&limit=${n}`)}function o(e,t,n){t.innerHTML=e.map(e=>`
  ...
  `)}var C=`quote`;async function w(){let e=localStorage.getItem(C),t=new Date().toISOString().split(`T`)[0];if(e){let n=JSON.parse(e);if(n.date===t){S(n.data);return}}try{let e=await v();localStorage.setItem(C,JSON.stringify({date:t,data:e})),S(e)}catch(e){console.error(e),n.default.error({title:`Error`,message:`Failed to load quote`,position:`topRight`}),S({author:`Unknown`,quote:`Stay active and take care of your health`})}}async function T(){try{w(),h(),d()}catch(e){console.error(e)}}T();
